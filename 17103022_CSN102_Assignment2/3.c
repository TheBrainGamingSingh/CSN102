...
void insert(char song[20],char singer[20],char movie[20],list** start) 
{
	...
	if(*start==NULL)
	  ...
	  else if (strcmpi((*start)->song_name,song)>0) 
	  {
     ...
     else
	 {
      ...
      while(temp->next!= NULL)
		  if(strcmpi(temp->next->song_name,song)>0)
           break;
 	     else temp=temp->next;
...
void play(list* start)
{
	...
	printf("\n"); 
	      printinfo(temp);
	      printf("\n1 Next Song\n2 Previous Song\n3 Print next n songs\n4 Print previous n songs\n5 Back\n");
	fflush(stdin);	  
	scanf("%d",&i);	  
	switch(i)
	{
		case 1 : if(temp->next!=NULL)
		         temp=temp->next;
			     else printf("\n\nEnd of tape,this is the last song in the list\n\n");
		         goto PLAY;
				 
		case 2 : if(temp->prev!=NULL)
		         temp=temp->prev; 
			     else printf("\n\nCan't go back,this is the first song in the list\n\n");
		         goto PLAY;
		...
		case 4 : printf("Enter n\n");
				 ...
				 for(int x=0;x<n2;x++)
				 {
					 if(walker1->prev!=NULL)
					 {
						 printinfo(walker1->prev);
